...
def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print_error("Usage: pm task-close <epic-name> <task-number>")
        print()
        print("Example: pm task-close user-auth-backend 2")
        sys.exit(1)
    ...
    with get_db() as db:
        ...
        if all_closed:
            ...
            if epic_status != 'closed':
                print(f"   Consider closing the epic: pm epic-close {epic_name}")

    print()
    print("Next steps:")
    print("   • View ready tasks: pm next")
    print(f"   • View epic status: pm epic-show {epic_name}")
    print("   • Daily standup: pm standup")
