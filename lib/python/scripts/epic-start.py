...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: pm epic-start <epic-name>")
        sys.exit(1)
    ...
    print()
    print("Next steps:")
    print(f"   View tasks: pm epic-show {epic_name}")
    print("   View ready tasks: pm next")
    print(f"   Start a task: pm task-start {epic_name} <task-number>")
    print()
    print("Note: Full worktree creation requires the bash version")
    print(f"      Run: bash .claude/scripts/pm/epic-start-db.sh {epic_name}")
