...
class CCPMDatabase:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to database file. If None, uses environment variable
                    CCPM_DB_PATH or defaults to ~/.claude/ccpm.db
        """
        if db_path is None:
            # Check for environment variable first (plugin mode)
            db_path = os.environ.get('CCPM_DB_PATH')
            if db_path is None:
                # Default to standard location (repository mode)
