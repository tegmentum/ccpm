...
def main():
    ...
    if not ready_tasks:
        ...
        print()
        print(f"Check status: ccpm:epic-show {epic_name}")
        sys.exit(0)
    ...
    if analyzed_count == 0:
        ...
        print(f"     Example: ccpm:issue-analyze <github-issue-number>")
        ...
        if choice == "1":
            ...
            for task in ready_tasks:
                if task.get('github_issue_number'):
                    print(f"  ccpm:issue-analyze {task['github_issue_number']}")
            sys.exit(0)
        elif choice == "3":
            print("Cancelled")
            sys.exit(0)
  ...
  prompt: |
    ...
    When complete:
    - Run tests
    - Mark task complete: ccpm:task-close {epic_name} {task_num}
""")
        print()
    ...
    print("  3. Monitor progress: ccpm:epic-status " + epic_name)
    print("  4. When agents complete: ccpm:epic-refresh " + epic_name)
    print()
