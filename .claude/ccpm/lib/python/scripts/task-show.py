...
def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print_error("Usage: ccpm:task-show <epic-name> <task-number>")
        print()
        print("Example: ccpm:task-show user-auth-backend 3")
        sys.exit(1)
    ...
    if task_status == 'open':
        if not dependencies or all(d['status'] == 'closed' for d in dependencies):
            print(f"  • Start this task: ccpm:task-start {epic_name} {task_number}")
        else:
            print("  • Cannot start - has unmet dependencies")
            print("  • View blocked tasks: ccpm:blocked")
        ...
        if dependents:
            print("  • Check unblocked tasks: ccpm:task-next")

    print(f"  • View epic: ccpm:epic-show {epic_name}")
    if github_issue:
