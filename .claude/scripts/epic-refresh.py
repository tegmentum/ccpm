...
def main():
    ...
    if len(sys.argv) < 2:
        print_error("Usage: ccpm:epic-refresh <epic-name>")
        sys.exit(1)
    ...
    if new_status == "closed":
        ...
        print()
        print("Next steps:")
        print(f"   Close epic: ccpm:epic-close {epic_name}")
    elif in_progress_tasks > 0:
        print(f"= Work in progress ({in_progress_tasks} task(s))")
        print()
        print("Next steps:")
        print(f"   View progress: ccpm:epic-show {epic_name}")
        print("   See next tasks: ccpm:task-next")
    else:
        print("=Ë Ready to start work")
        print()
        print("Next steps:")
        print("   See next tasks: ccpm:task-next")
        print(f"   Start task: ccpm:task-start {epic_name} <num>")
    print()
