...
def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print_error("Usage: pm task-start <epic-name> <task-number>")
        print()
        print("Example: pm task-start user-auth-backend 3")
        sys.exit(1)
    ...
    if current_status == 'closed':
        ...
        print(f"   {task_name}")
        print()
        print("To reopen, update status manually with: pm task-update")
        sys.exit(1)
    ...
    print()
    print("Next steps:")
    print(f"   • View task details: pm task-show {epic_name} {task_number}")
