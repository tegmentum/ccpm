...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print_error("Usage: ccpm:issue-start <issue-number>")
        sys.exit(1)
    ...
    if not check_worktree(epic_name):
        ...
        print(f"  Run: ccpm:epic-start {epic_name}")
        sys.exit(1)
    ...
    if not analysis_path:
        ...
        print()
        print("Options:")
        print(f"  1. Analyze first: ccpm:issue-analyze {issue_number}")
...
""")
    ...
    print(f"   2. Monitor progress: ccpm:epic-status {epic_name}")
    print(f"   3. When done: ccpm:task-close {epic_name} {task_number}")
    print()
