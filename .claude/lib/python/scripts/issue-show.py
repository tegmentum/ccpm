...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("Usage: ccpm:issue-show <issue-number>")
        sys.exit(1)
    ...
    with get_db() as db:
        ...
        if not tasks:
            ...
            print("Try: ccpm:import  or  ccpm:github-sync")
            sys.exit(1)
    ...
    print("Quick Actions:")
    if task['status'] == 'open':
        print(f"   Start work: ccpm:task-start {task['epic_name']} <task-number>")
    elif task['status'] == 'in_progress':
        print(f"   Complete: ccpm:task-close {task['epic_name']} <task-number>")
    print(f"   Sync to GitHub: ccpm:github-sync")
