...
1. **Verify epic exists:**
   - Check if `epics/$ARGUMENTS/epic.md` exists
   - If not found, tell user: "❌ Epic not found: $ARGUMENTS. First create it with: /ccpm:prd-parse $ARGUMENTS"
   - Stop execution if epic doesn't exist

2. **Check for existing tasks:**
   - Check if any numbered task files (001.md, 002.md, etc.) already exist in `epics/$ARGUMENTS/`
...
3. **Validate epic frontmatter:**
   ...
   - If invalid, tell user: "❌ Invalid epic frontmatter. Please check: epics/$ARGUMENTS/epic.md"
...
### 1. Read the Epic
- Load the epic from `epics/$ARGUMENTS/epic.md`
...
Task:
  ...
  prompt: |
    ...
    For each task:
    1. Create file: epics/$ARGUMENTS/{number}.md
...
Save tasks as: `epics/$ARGUMENTS/{task_number}.md`
...
3. Suggest next step: "Ready to sync to GitHub? Run: /ccpm:github-sync"
