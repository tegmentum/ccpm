...
```bash
bash .claude/ccpm/scripts/init.sh
```
...
1. **Check Context Directory**
   ```bash
   ls -la .claude/ccpm/context/ 2>/dev/null
   ls -1 .claude/ccpm/context/*.md 2>/dev/null | wc -l
   ```
...
4. **Create Context Files**
   ...
   Create `.claude/ccpm/context/` directory and generate these 9 files with frontmatter:
