...
```bash
# Epic must exist
test -f epics/$ARGUMENTS/epic.md || echo "❌ Epic not found. Run: /ccpm:prd-parse $ARGUMENTS"

# Check for existing tasks
if ls epics/$ARGUMENTS/[0-9]*.md 2>/dev/null | grep -q .; then
...
if grep -q "github:" epics/$ARGUMENTS/epic.md; then
  echo "⚠️ Epic already synced to GitHub."
  echo "Use /ccpm:github-sync to update."
  exit 1
fi
```
