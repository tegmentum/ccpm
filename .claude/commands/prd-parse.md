...
2. **Verify PRD exists:**
   - Check if `prds/$ARGUMENTS.md` exists
...
3. **Validate PRD frontmatter:**
   ...
   - If frontmatter is invalid or missing, tell user: "❌ Invalid PRD frontmatter. Please check: prds/$ARGUMENTS.md"
   - Show what's missing or invalid

4. **Check for existing epic:**
   - Check if `epics/$ARGUMENTS/epic.md` already exists
...
5. **Verify directory permissions:**
   - Ensure `epics/` directory exists or can be created
...
### 1. Read the PRD
- Load the PRD from `prds/$ARGUMENTS.md`
...
Create the epic file at: `epics/$ARGUMENTS/epic.md` with this exact structure:
...
- `epics/$ARGUMENTS/` (directory)
- `epics/$ARGUMENTS/epic.md` (epic file)
...
1. Confirm: "✅ Epic created: epics/$ARGUMENTS/epic.md"
2. Show summary of:
   - Number of task categories identified
   - Key architecture decisions
   - Estimated effort
3. Suggest next step: "Ready to break down into tasks? Run: /ccpm:epic-decompose $ARGUMENTS"
